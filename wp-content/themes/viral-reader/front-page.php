...
$vr_hero  = ! empty( $vr_posts ) ? $vr_posts[0] : null;
?>
	...
	<div class="vr-container">
		<section class="home-hero">
			...
			if ( $vr_hero_id ) {
				...
				$vr_hero_ext = get_post_meta( $vr_hero->ID, '_wpap_image_url', true );
				if ( is_string( $vr_hero_ext ) && '' !== $vr_hero_ext && preg_match( '#^https?://#i', $vr_hero_ext ) ) {
...
<?php else : ?>
	<?php /* No posts yet: still give the front page an h1 for a valid outline. */ ?>
...
<?php $vr_cats = get_categories( array( 'orderby' => 'count', 'order' => 'DESC', 'number' => 8, 'hide_empty' => true ) ); ?>
			...
			<div class="category-list">
				<?php foreach ( $vr_cats as $vr_c ) : ?>
					...
					<a class="category-card" href="<?php echo esc_url( is_wp_error( $vr_cl ) ? '#' : $vr_cl ); ?>">
						<span class="count"><?php echo esc_html( sprintf( _n( '%d story', '%d stories', (int) $vr_c->count, 'viral-reader' ), (int) $vr_c->count ) ); ?></span>
...
<?php if ( count( $vr_posts ) > 1 ) : ?>
	...
	<section class="section">
		<div class="vr-container">
			...
			<div class="post-grid">
				<?php
				foreach ( array_slice( $vr_posts, 1 ) as $vr_p ) {
