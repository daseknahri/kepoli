...
- The setup box includes automation buttons for `Completeaza automat`, `Sugereaza categorie`, `Extrage schema reteta`, `Genereaza excerpt`, `Genereaza meta description`, `Sugereaza linkuri interne`, and `Genereaza meta imagine`.
...
8. Add a featured image, then click `Genereaza meta imagine` if you want to refresh alt text, title, caption, and Media Library description specifically for that image.
9. For recipes, click `Extrage schema reteta` after you write the structured content. Kepoli will look for sections like `Ingrediente` and `Mod de preparare` and move them into the recipe schema fields.
10. For a long article, click `2 parti` or `3 parti` in the toolbar.
11. Watch the live checklist in `Kepoli post setup` and complete any missing items.
