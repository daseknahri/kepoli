...
- The `Kepoli post setup` box lets the writer choose `Reteta` or `Articol`, write a meta description, add related post slugs, complete featured-image metadata, and complete recipe structured data.
- The setup box includes automation buttons for `Genereaza meta description`, `Sugereaza linkuri interne`, and `Genereaza meta imagine`.
- If the writer saves a post with empty meta description, related slugs, or featured-image metadata, Kepoli fills sensible defaults from the title/content and current post library.
...
5. Click `Genereaza meta description` and `Sugereaza linkuri interne` if you want Kepoli to prefill SEO and internal links from the pasted content.
